...
app.post('/LeeHan/name_decide', (req, res) =>{
    ...
    connection.query('INSERT INTO leehan_account_name (id, player_name) VALUES (?, ?)', [id, player_name], (err, results) => {
        if (err) {
    ...
    connection.query('SELECT id, password FROM leehan_account WHERE id = ?', [id], (err, results, fields) => {
        ...
        if(results[0] !== undefined){
            ...
            if (dataArray.password !== password){
                ...
                console.log('로그인은 성공함');
                connection.query('SELECT id FROM leehan_account_name WHERE id = ?', [id], (err, results, fields) =>{
                    const token = jwt.sign({ dataArray }, secretkey, { expiresIn: '1h' });
                     res.status(200).json({ token, id: id, player_name: results[0].player_name});
                });
                     
                // if(!dataArray.player_name){
                //     console.log('true보냄');
                //     return res.status(401).json({ name_is_null: true });
                // }
                // else{
                //     console.log('계정정보보냄')
                //     const token = jwt.sign({ dataArray }, secretkey, { expiresIn: '1h' });
...
app.post('/LeeHan/sign_up', (req, res) => {
    ...
    connection.query('INSERT INTO leehan_account (id, password) VALUES (?, ?)', [id, password], (err, results) => {
        ...
        res.status(200).json({ message: '회원가입에 성공하였습니다.', name_is_null: true });
    });
...
// const wss = new WebSocket.Server({port:3000}, () =>{            //소캣을 포트 8000번에 시작 시킨다.
//     console.log('서버 시작');
// });
